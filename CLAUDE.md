...
│   │   └ consistency-verification.md # Points to ecf-verify
│   └── references/
...
└── ecf-verify/SKILL.md  # Standalone: Verify alignment (~179 lines)
```
...
| ecf-verify | `/ecf-verify` or `Skill("ecf-verify")` | Verify spec↔design↔code↔test alignment |
...
- **禁止直接编辑 SKILL.md** - 必须通过 `skill-creator` TDD 流程
...
**标准流程**: `/opsx:propose → skill-creator → skill-quality-verification → /opsx:archive → ce:compound`
