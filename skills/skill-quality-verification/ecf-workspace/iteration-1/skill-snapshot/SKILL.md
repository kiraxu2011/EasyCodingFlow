...
digraph preflight {
    ...
    summary [label="输出依赖状态汇总", shape=box];
...
| 依赖 | 状态 | Fallback 策略 |
|------|------|---------------|
| OpenSpec | 缺失 | Brainstorming 替代 Contract Layer |
| CE | 未安装 | 直接写入 docs/solutions/ |
| Superpowers | frad 未安装 | 官方版本 fallback |
| Superpowers | 完全未安装 | **Critical**: 阻止流程 |
...
**遇到以上**: 从 writing-plans Phase 6 重新开始，强制使用 `/ecf-execute`。
...
| ecf-consistency-verification | `/ecf-consistency-verification` | 一致性验证 |